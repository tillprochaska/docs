...
{% tab title="macOS" %}
When running Docker on macOS, Docker uses a virtual machine as the container host. Therefore, you need to run the `sysctl` command inside of the Docker virtual machine. In order to log in to the virtual machine, execute:

```
docker run -it --rm --privileged --pid=host justincormack/nsenter1
```

Then execute the command to allow memory mapping:
